...
import modules.layers.functional as LF
...
class Rpool(nn.Module):
    ...
    def forward(self, x, aggregate=True):
        # features -> roipool
        # x: (bs, C, H, W)
        # roipool 在特征图上按照一定的规则划定roi(x个)， 并对每一个roi进行pool操作，最终将池化结果concatenate起来 (bs, x, C, 1, 1)
        o = LF.roipool(x, self.rpool, self.L, self.eps) # size: #im, #reg, D, 1, 1

        # concatenate regions from all images in the batch
        s = o.size()   # (bs, x, C, 1, 1)
        o = o.view(s[0]*s[1], s[2], s[3], s[4]) # size: (bs * x, C, 1, 1)
        ...
        o = o.view(s[0], s[1], s[2], s[3], s[4])  # (bs, x, C, 1, 1)

        # aggregate regions into a single global vector per image
        if aggregate:
            # rvecs -> sumpool -> norm
            # 这里已经norm过一次了，外面又norm了一次，感觉这里的norm没什么必要, 所以擅自去掉了了。
            # o = self.norm(o.sum(1, keepdim=False)) # size:(bs, C, 1, 1)
            #自己改过的：
            o = o.sum(1, keepdim=False)
        return o
