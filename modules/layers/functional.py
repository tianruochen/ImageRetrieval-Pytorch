...
# 全局最大池化
def mac(x):
    return F.max_pool2d(x, (x.size(-2), x.size(-1)))    # (x.size(-2), x.size(-1))是kernel的大小
    # return F.adaptive_max_pool2d(x, (1,1)) # alternative    (1,1) 是最终输出的大小
...
def contrastive_loss(x, label, margin=0.7, eps=1e-6):
    # x is (embedding_dim, bs)
    dim = x.size(0)   # embedding_dim
    nq = torch.sum(label.data==-1)  # number of tuples
    S = x.size(1) // nq  # number of images per tuple including query: 1+1+n

    # (embedding_dim, nq) --> (nq, embedding_dim) --> (nq, embedding_dim * (1+n)) --> ((1+n)*nq, embedding_dim)
